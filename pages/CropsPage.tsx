...
import type { CropsPageData, Category } from '../types.ts';
import LoadingSpinner from '../components/LoadingSpinner.tsx';
import Card from '../components/Card.tsx';
import ImageGenerator from '../components/ImageGenerator.tsx';

const CropsPage: React.FC = () => {
  const { data, loading, error } = useData<CropsPageData>('crops');
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            {filteredCategories.length > 0 ? (
              ...
              <div className="lg:col-span-2 text-center py-10 bg-white dark:bg-gray-800 rounded-lg shadow-md">
                  <p className="text-gray-500 dark:text-gray-400">No matching crops found.</p>
              </div>
            )}
        </div>
    </div>
